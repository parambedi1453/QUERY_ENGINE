...
int check_url(char url[])
{
		...
		if(!system(url_temp)) //return 0 for system command inbuilt
		{
			/*char seed_url[strlen(url)+1];
										...
										{
											printf("BASE URL IS NOT SAME AS SEED URL\n");
											return 0;
										}
										else*/
											return 1;
...
void storage()
{
	FILE *store = fopen("/home/parambedi/search_engine/storage.txt","w");
	node *read = head;
	while(read!=NULL)
	{
		fprintf(store,"%d ",read->node_key);
		fprintf(store,"%s ",read->node_url);
		fprintf(store,"%d ",read->node_depth);
		fprintf(store,"%d \n",read->node_flag);
		read=read->next;
	}
...
//*****************************************************************************
void collect_next_url(char *read_file,int file_size,char *url,int dep)//duplicate
{
	int i,p=0,k;
	char **result=NULL;
...
void create_permanent_file(int dep,char *url)
{
      ...
      strcat(file_name,"/home/parambedi/search_engine/");
      strcat(file_name,indexarr);
      strcat(file_name,".txt");
      FILE *temp = fopen("/home/parambedi/search_engine/temp.txt","r");
...
//*********************new_link_list*****************************
void new_link_list(char s[])
{
	int i=0,k=0,c=1;
	int old_flag,old_depth,old_key;
	char old_url[150];
	char w[150]={0};
	while(s[i]!='\0')
	{
	...
	int index=old_key;
	 node *temp=NULL,*p=NULL;
	 temp=(node*)calloc(1,sizeof(node));
	 temp->node_flag=old_flag;
	 temp->node_key=old_key;
	 strcpy(temp->node_url,old_url);
	 temp->node_depth=old_depth;
	 if(head==NULL)
		{
				head=temp;
				h[index].first=temp;
...
//********************READ STORAGE*********************************************
void read_storage(node *head)
{
	FILE *open = fopen("/home/parambedi/search_engine/storage.txt","r");
	char ch;
	char *niyati=(char *)calloc(1,250);
	int i=0;
    ...
    else
    {
        int result_check_url=check_url(argv[1]);
          ...
          else
          {
					...
					char tell;
					printf("\n DO YOU WANT TO RETRIEVE PREVIOUS DATA \n");
					scanf("%c",&tell);
					if(tell=='N'||tell=='n')
					{
						int dep=atoi(argv[2]);
